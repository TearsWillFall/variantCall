...
call_variants_strelka_parallel=function(bin_path="tools/strelka-2.9.10/build/bin/configureStrelkaSomaticWorkflow.py",bin_path2="~/tools/manta-1.6.0/build/bin/configManta.py",bam_dir="",ref_genome="",output_dir="",patient_id="",germ_pattern="GL",verbose=FALSE,targeted=FALSE,jobs=1,threads=3,exec_options="local"){
  ...
  pbapply::pblapply(X=1:length(tumor_bams),FUN=function(x){
    call_sv_manta(bin_path=bin_path,tumor_bam=tumor_bams[x],normal_bam=normal_bam,ref_genome=ref_genome,output_dir=out_file_dir,verbose=verbose,targeted=targeted,threads=threads);
    call_variants_strelka(bin_path=bin_path,tumor_bam=tumor_bams[x],normal_bam=normal_bam,ref_genome=ref_genome,output_dir=out_file_dir,verbose=verbose,indel_candidates=paste0(out_file_dir,"/",ULPwgs::get_sample_name(x),"_MANTA_SV_SOMATIC/candidateSmallIndels"),targeted=targeted,threads=threads,exec_options=exec_options)},cl=cl)
  on.exit(parallel::stopCluster(cl))
}
