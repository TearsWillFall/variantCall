...
vcf_communality <- function(bin_path="tools/bcftools/bcftools",vcf="",vcf2="",output_name="",output_dir="",verbose=FALSE){
  ...
  if (!dir.exists(out_file_dir) & !out_file_dir==""){
    ...
    #   sample_name=ULPwgs::get_sample_name(vcf)
    # }
  generate_sets(bin_path=bin_path,vcf=c(vcf,vcf2),filter="PASS",output_dir=paste0(out_file_dir,paste0(ULPwgs::get_sample_name(vcf),"_AND_",ULPwgs::get_sample_name(vcf2))),verbose=verbose,set_names=c(ULPwgs::get_sample_name(vcf),ULPwgs::get_sample_name(vcf2)))
