...
plot_allelic_imbalance=function(clonet_dir="",sample_data="",output_dir="",gene_data=""){
    ...
    for (x in as.character(unique(plasma$Timepoint_ID))){
      ...
      write.table(paste0(out_file_dir,"/",unique(tissue$Patient_ID),".",x,".Allelic_Imbalance_CLONET_plasma.txt",sub_tissue,quote=FALSE,row.names=FALSE,col.names=TRUE))    }
    ...
    if(dim(tissue)[1]>0){
      for (x in unique(tissue$Anatomy)){
        ...
        write.table(paste0(out_file_dir,"/",unique(tissue$Patient_ID),".",x,".Allelic_Imbalance_CLONET_tissue.txt",sub_tissue,quote=FALSE,row.names=FALSE,col.names=TRUE))
      }
    }
}
